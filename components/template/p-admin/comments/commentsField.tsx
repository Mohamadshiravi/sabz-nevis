...
export default function CommentField({
    ...
    <div className="border border-zinc-200 dark:border-zinc-800 p-4 rounded-xs">
      ...
      {data.replyTo && (
        <div className="flex flex-col gap-3 mt-4">
          <span className="text-sm text-my-text-500">ریپلای به :</span>
          <CommentReplyField data={data.replyTo} />
        </div>
      )}
  ...
  async function AcceptCommentHandler() {
    setLoading(true);
    try {
      const res = await axios.put(`/api/post/comment/accept/${data._id}`);
      SendSucToast("کامنت تایید شد");
      setLoading(false);
      reRenderComments();
    } catch (error) {
      setLoading(false);
      SendErrorToast("مشکلی در تایید کامنت وجود دارد");
    }
  }
  ...
  async function DeleteCommentHandler() {
    setLoading(true);
    try {
      const res = await axios.delete(`/api/post/comment/${data._id}`);
      SendSucToast("کامنت حذف شد");
      setLoading(false);
      reRenderComments();
    } catch (error) {
      setLoading(false);
      SendErrorToast("مشکلی در حذف کامنت وجود دارد");
    }
  }
}
